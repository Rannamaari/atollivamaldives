...
<article><a href="{{ $product->publicUrl() }}"><div class="photo"><img src="{{ $image }}" alt="{{ $product->name }} in the Maldives" loading="lazy" decoding="async"><small>{{ str_pad($loop->iteration,2,'0',STR_PAD_LEFT) }}</small></div><div class="meta"><span>{{ strtoupper($product->type->label()) }}</span><span>{{ $product->island }}</span></div><h3>{{ $product->name }}</h3><div class="foot"><span>{{ $product->tagline }}</span><b>FROM {{ $product->currency }} {{ number_format($product->price_from) }}</b></div></a></article>
